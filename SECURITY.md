...
If you discover a security vulnerability in Syll, please report it by:
...
chmod 600 ~/.syll/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.syll/config.json` with file permissions set to `0600`
...
- ✅ Run Syll with a dedicated user account
...
- Keep authentication data in `~/.syll/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for Syll and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash syll
   sudo -u syll syll wake
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.syll
   chmod 600 ~/.syll/config.json
   chmod 700 ~/.syll/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.syll/logs/syll.log
   ```
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.syll/logs/syll.log
   ```
...
- GitHub Security Advisories: https://github.com/THU-SAGE/syll/security/advisories
- Release Notes: https://github.com/THU-SAGE/syll/releases
